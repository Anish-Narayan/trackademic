...
const StudentOnboarding = () => {
  ...
  // Initialize state, leaving department and batch empty for the dropdown placeholders.
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    // Validation checks that a value has been selected from the dropdowns.
    if (!formData.displayName || !formData.department || !formData.batch || !formData.rollNumber) {
    ...
    try {
      ...
      navigate('/dashboard', { replace: true }); // Redirect to the main redirector.
    } catch (err) {
  ...
  return (
    <div className="container mx-auto p-4 max-w-md text-center mt-16">
      <h2 className="text-3xl font-bold text-[#004d99] mb-4">Welcome, Student!</h2>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-md text-left">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            <label htmlFor="batch" className="block text-gray-700 font-semibold mb-2">Batch</label>
            <select
                id="batch"
                name="batch"
